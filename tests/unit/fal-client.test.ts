...
describe('fal-client image generation', () => {
  ...
  it('adjusts duration when fal returns bare numeric permitted values in validation message', async () => {
    const validationError: any = new Error('ValidationError: Unprocessable Entity')
    validationError.status = 422
    validationError.body = {
      detail: [
        {
          loc: ['body', 'duration'],
          msg: 'Input should be 6, 8 or 10',
          type: 'literal_error',
          input: '6',
          ctx: { expected: '6, 8 or 10' },
        },
      ],
    }

    uploadMock.mockRejectedValueOnce(Object.assign(new Error('lookup failed'), { cause: { code: 'ENOTFOUND' } }))
    subscribeMock
      .mockRejectedValueOnce(validationError)
      .mockResolvedValueOnce({ data: { video: { url: 'https://example.com/video.mp4' } } })

    const hugeDataUrl = `data:image/png;base64,${Buffer.alloc(1024, 1).toString('base64')}`
    const url = await falGenerateVideo({
      endpoint: 'fal-ai/unknown/image-to-video',
      prompt: 'test',
      sourceImageUrl: hugeDataUrl,
      duration: '6',
    })
    ...
    expect(subscribeMock).toHaveBeenCalledTimes(2)
    expect(subscribeMock.mock.calls[0][1].input.image_url).toBe(hugeDataUrl)
    expect(subscribeMock.mock.calls[0][1].input.duration).toBe('6')
    expect(subscribeMock.mock.calls[1][1].input.duration).toBe(6)
  })
